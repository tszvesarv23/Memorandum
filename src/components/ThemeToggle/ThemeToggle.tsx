...
import { DEFAULT_THEME, type Theme, THEME_COOKIE } from "@/lib/theme";
import styles from "./ThemeToggle.module.css";

function resolveEffectiveTheme(initial?: Theme): Theme {
  if (typeof document !== "undefined") {
    const dom = document.documentElement.dataset.theme;
    if (dom === "light" || dom === "dark") return dom;
  }
  return initial ?? DEFAULT_THEME;
}
...
export function ThemeToggle({ initialTheme }: { initialTheme?: Theme }) {
  const [theme, setTheme] = useState<Theme>(() => resolveEffectiveTheme(initialTheme));
  const [mounted, setMounted] = useState(false);

  // Sincroniza con cualquier cambio externo del DOM (script de inicialización)
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  return (
    ...
    >
      ...
      {!mounted && <span className={styles.srOnly}>{label}</span>}
    </button>
  );
}
